...
class Test_Product(unittest.TestCase): # Created a test suite (a collection of test cases, test suites, or both) that groups all the product-related tests.
 # 1. Check normal product creation
    def test_product_creation(self):
        product = Product("P001", "Milk", 10, 70.0, date.today() + timedelta(days=5))
    ...
    def test_update_quantity(self):
        product = Product("P002", "Plastic Bag", 5, 4.0, date.today() + timedelta(days=10))
        product.update_quantity(10)  
        self.assertEqual(product.quantity, 15)

    # 3. Update product price
    def test_update_price(self):
        product = Product("P003", "Pomegranate", 5, 100.0, date.today() + timedelta(days=10))
    ...
    def test_invalid_quantity_update(self):
        product = Product("P006", "Eggs", 50, 5.0, date.today() + timedelta(days=10))
    ...
    def test_invalid_price_update(self):
        product = Product("P007", "Butter", 10, 140.0, date.today() + timedelta(days=10))
