...
-- TABLE 1: Products
-- Stores one row per unique product (name, category, unit). It can have many batches.
...
-- TABLE 2: Batches
-- Each batch has its own price, quantity, and expiry date.FK → Products ensures every batch belongs to a known product.
...
-- TABLE 3: Sales
-- Records every individual sale transaction. FK (product_id, batch_number); Batches ensures stock exists before we make sales.
...
CREATE TABLE Sales (
    ...
    CONSTRAINT ck_sales_price       CHECK (price_per_unit > 0),
    CONSTRAINT ck_sales_total       CHECK (total_amount > 0),
    CONSTRAINT ck_sales_id          CHECK (sale_id REGEXP '^S[0-9]{3}$')
);


-- TABLE 4: RemovedProducts
-- Audit log of expired products removed from inventory.Batches (SET NULL on delete so the log record survives even if the batch is purged).
...
CREATE TABLE RemovedProducts (
    ...
    price_at_removal    DECIMAL(10,2)   NOT NULL,
    ...
    CONSTRAINT fk_removed_batch     FOREIGN KEY (product_id, batch_number)
                                    ...
                                    ON DELETE SET NULL
                                    ON UPDATE CASCADE,
    CONSTRAINT ck_removed_qty       CHECK (quantity_removed > 0),
    CONSTRAINT ck_removed_reason    CHECK (removal_reason IN ('Expired', 'Damaged', 'Recalled', 'Other'))
);
